...
export default function BankerDashboardPage() {
  ...
  const { dashboardData, loading: dataLoading } = useDashboardData(user?.roleName || 'BANKER');
  ...
  if (authLoading || dataLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-teal-900/20 to-gray-900 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-teal-500 mx-auto mb-4"></div>
          <p className="text-white text-lg">Loading Banker Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <DashboardLayout role={user.roleName || 'BANKER'}>
