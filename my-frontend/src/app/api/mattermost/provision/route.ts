  ...
  try {
    ...
    if (!mmUser) {
      const username = (user.username || user.name || user.email.split('@')[0])
        .toLowerCase()
        .replace(/[^a-z0-9._-]/g,'')
        .slice(0, 20); // Limit username length
        
      // Use demo password for development; in production, sync from secure credential store
      ...
      mmUser = await mmCreateUser({ 
        email: user.email, 
        username: username || `u${user.id}`, 
        password, 
        first_name: user.name?.split(' ')?.[0] || '', 
        last_name: user.name?.split(' ')?.slice(1)?.join(' ') || '' 
      });
      // SECURITY NOTE: In production, implement secure password synchronization
    }
