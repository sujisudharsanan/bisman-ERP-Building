...
export default function LegalDashboardPage() {
  ...
  const { dashboardData, loading: dataLoading } = useDashboardData(user?.roleName || 'LEGAL');

  React.useEffect(() => {
    if (!authLoading) {
      if (!user) {
        router.push('/auth/login');
      } else if (user.roleName !== 'LEGAL') {
  ...
  if (authLoading || dataLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900/20 to-gray-900 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-slate-500 mx-auto mb-4"></div>
          <p className="text-white text-lg">Loading Legal Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <DashboardLayout role={user.roleName || 'LEGAL'}>
