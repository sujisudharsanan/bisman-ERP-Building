/**
 * Common Module - Pages Index
 * Export all common pages for easy importing
 */

export { default as AboutMePage } from './about-me';
export { default as ChangePasswordPage } from './change-password';
export { default as NotificationsPage } from './notifications';
export { default as HelpCenterPage } from './help-center';
export { default as UserSettingsPage } from './user-settings';
export { default as SecuritySettingsPage } from './security-settings';
export { default as MessagesPage } from './messages';
export { default as DocumentationPage } from './documentation';
export { default as PaymentRequestPage } from './payment-request';
export { default as CommonCalendarPage } from './calendar-common';
