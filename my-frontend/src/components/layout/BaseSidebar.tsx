	...
	return (
		<>
			...
			>
				...
				{/* Sidebar Navigation */}
								<nav 
									className={`flex-1 overflow-y-auto space-y-1 ${collapsed && !isMobile ? 'p-2' : 'p-4'}`} 
										...
										return (
											<Link
												...
												className={`
													...
													${collapsed && !isMobile ? 'justify-center px-2 py-2.5' : 'px-3 py-3'}
												`}
												title={item.label}
											...
											>
												<IconComp 
													size={collapsed && !isMobile ? 22 : 20} 
													className={`flex-shrink-0 ${isActive ? 'text-indigo-600 dark:text-indigo-400' : ''}`} 
													aria-hidden="true" 
												/>
												{(!collapsed || isMobile) && (
				...
				{/* Sidebar Footer */}
								<div className={`border-t border-gray-200 dark:border-gray-800 ${collapsed && !isMobile ? 'p-2' : 'p-4'}`}>
