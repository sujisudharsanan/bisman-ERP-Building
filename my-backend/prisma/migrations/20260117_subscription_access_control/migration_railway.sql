-- ============================================================================
-- SUBSCRIPTION ACCESS CONTROL - COMPLETE MIGRATION FOR RAILWAY
-- Created: 2026-01-17
-- Purpose: Full migration including enums and all tables
-- ============================================================================

-- 1. Create enum types if they don't exist
DO $$ BEGIN
    CREATE TYPE module_access_level AS ENUM ('none', 'read', 'full');
EXCEPTION
...
-- 2. Create plan_module_access table if it doesn't exist (production table)
...
-- 3. Draft table for plan module access (staging changes)
...
-- 4. Draft table for plan feature controls (staging changes)
...
-- 5. Tenant module overrides (per-tenant access beyond plan)
CREATE TABLE IF NOT EXISTS tenant_module_overrides (
  ...
  override_type       VARCHAR(30) NOT NULL DEFAULT 'grant',
...
-- 6. Publish audit log (tracks all publish events)
...
-- 8. Seed draft tables from production data (if production tables have data)
...
-- Add comments
COMMENT ON TABLE plan_module_access IS 'Production table: Controls which modules are available in each subscription plan';
