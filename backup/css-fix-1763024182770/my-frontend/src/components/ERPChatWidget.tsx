...
export default function ERPChatWidget({ userName }: { userName?: string }) {
  ...
  // Fetch unread message count (migrated to safeFetch with timeout + resilience)
  useEffect(() => {
    let cancelled = false;
    const fetchUnreadCount = async () => {
      try {
        const r = await safeFetch('/api/messages/unread-count', { timeoutMs: 5000 });
        if (!r.ok) return;
        const data = await r.json();
        if (!cancelled && typeof data.count === 'number') {
          setUnreadCount(data.count);
          if (data.count > 0) setIconState('notify');
        }
      } catch {
        // silent fail – do not spam console for polling
      }
    };
    fetchUnreadCount();
  ...
  safeFetch('/api/messages/mark-read', { method: 'POST', timeoutMs: 4000 }).catch(() => {});
    }
  }, [open]);
