...
export default function StoreInchargeDashboardPage() {
  const { user, loading: authLoading } = useAuth();
  const router = useRouter();
  
  const { dashboardData, loading: dataLoading } = useDashboardData(user?.roleName || 'STORE_INCHARGE');

  React.useEffect(() => {
    if (!authLoading) {
      if (!user) {
        router.push('/auth/login');
      } else if (user.roleName !== 'STORE_INCHARGE' && user.roleName !== 'STORE INCHARGE') {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-cyan-900/20 to-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white text-lg">Loading Store Incharge Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <DashboardLayout role={user.roleName || 'STORE_INCHARGE'}>
