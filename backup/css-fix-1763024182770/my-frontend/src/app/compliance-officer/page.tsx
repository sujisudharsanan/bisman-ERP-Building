...
export default function ComplianceOfficerDashboardPage() {
  ...
  const { dashboardData, loading: dataLoading } = useDashboardData(user?.roleName || 'COMPLIANCE');

  React.useEffect(() => {
    if (!authLoading) {
      if (!user) {
        router.push('/auth/login');
      } else if (user.roleName !== 'COMPLIANCE') {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-red-900/20 to-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white text-lg">Loading Compliance Dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <DashboardLayout role={user.roleName || 'COMPLIANCE'}>
