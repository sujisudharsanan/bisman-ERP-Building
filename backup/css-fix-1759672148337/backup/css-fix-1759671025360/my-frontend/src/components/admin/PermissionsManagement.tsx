...
import { usePermissions, useRoles, useRoutes, useActions } from '../../hooks/useRBAC'
import { Shield, Users, Route, Activity, Check, X, Edit, Save, Filter } from 'lucide-react'
...
export default function PermissionsManagement({ searchTerm }: PermissionsManagementProps) {
  ...
  const { permissions, loading, error, updatePermission } = usePermissions(selectedRole)
  ...
  const permissionsByRole = filteredPermissions.reduce((acc: any, permission: Permission) => {
  ...
  const modules = [...new Set(routes.map((route: any) => route.module).filter(Boolean))]

  const handlePermissionToggle = (permission: Permission) => {
    const key = `${permission.roleId}-${permission.actionId}-${permission.routeId}`
    const currentValue = pendingChanges.has(key) ? pendingChanges.get(key) : permission.granted
  ...
  const handleSavePermissions = async () => {
    try {
      const savePromises = Array.from(pendingChanges.entries()).map(([key, granted]) => {
  ...
  const getEffectivePermissionValue = (permission: Permission) => {
    const key = `${permission.roleId}-${permission.actionId}-${permission.routeId}`
    return pendingChanges.has(key) ? pendingChanges.get(key) : permission.granted
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage role-based permissions for routes and actions</p>
        </div>
      ...
      {viewMode === 'grid' && (
        <div className="space-y-6">
          {Object.entries(permissionsByRole).map(([roleName, rolePermissions]: [string, any]) => (
            <div key={roleName} className="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          ...
          {filteredPermissions.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {searchTerm ? 'Try adjusting your search terms or filters' : 'No permissions are configured'}
              </p>
            </div>
          )}
        </div>
