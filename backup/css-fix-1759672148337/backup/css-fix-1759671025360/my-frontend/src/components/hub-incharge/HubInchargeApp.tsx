    ...
    <div className="p-4 sm:p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white shadow rounded-2xl p-4">
          ...
          <p className="text-2xl font-bold text-orange-600">{data?.approvals?.filter((a: Approval) => a.status === 'pending').length || 0}</p>
...
const AboutMePage = ({ data, refetch }: { data: HubInchargeData | null; refetch?: () => void }) => {
  ...
  const employees = React.useMemo(() => [
    {
      ...
      details: [
        ...
        { label: "Designation", value: data?.profile?.role || "Hub Incharge" },
        { label: "Joining Date", value: "2020-03-15" },
        { label: "Date of Birth (DOB)", value: "1990-07-20" },
        { label: "Official Email", value: data?.profile?.contact || "staff@business.com" },
        { label: "Official Phone", value: "+91 9876543210" },
        { label: "Blood Group", value: "O+" },
        { label: "Home Phone Number", value: "+91 9876543211" },
        { label: "Current Address", value: "123 Corporate Lane, Business District" },
        { label: "State / Pincode", value: "Maharashtra 400001" },
        { label: "House Location (Geocode)", value: "Lat: 19.0760, Lon: 72.8777" }
      ],
      education: {
      ...
      awards: {
        ...
        items: Array.isArray(data?.profile?.recognition) && data.profile.recognition.length > 0
          ? data.profile.recognition.map((award: string) => `Achievement: ${award}`)
          : [
    ...
    // Sample team members
    {
      ...
      about: "Dedicated Assistant Manager with 5 years of experience in fuel operations and team management.",
      details: [
    ...
    },
    {
      ...
      name: "Priya Sharma",
      role: "Supervisor",
      photo: "https://images.unsplash.com/photo-1494790108755-2616b612b786?ixlib=rb-4.0.3&auto=format&fit=crop&w=200&h=200&q=80",
      about: "Experienced supervisor ensuring quality operations and customer satisfaction.",
      details: [
    ...
    },
    {
      ...
      about: "Detail-oriented cashier with excellent customer service skills and accuracy in transactions.",
      details: [
    ...
    },
    {
      ...
      name: "Sunita Reddy",
      role: "Safety Officer",
      photo: "https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-4.0.3&auto=format&fit=crop&w=200&h=200&q=80",
      about: "Certified safety officer ensuring compliance with all safety regulations and protocols.",
      details: [
  ...
  React.useEffect(() => {
    const loadProfilePicture = async () => {
      try {
        ...
        console.log('Could not load profile picture:', errMsg);
      }
    };
            ...
            <div className="profile-left-column">
              <div className="profile-card photo-block">
                <Image
                  ...
                  height={180}
                  unoptimized
                  style={{ margin: '0 auto 15px auto', display: 'block', borderRadius: '50%', objectFit: 'cover', border: '4px solid #3B82F6' }}
                />
                <div 
                ...
                >
                  <span>
                    {uploading ? 'Uploading...' : 'Click to Upload/Change Photo'}
                  </span>
                  <input 
                    type="file" 
              ...
              <div className="details-wrapper">
                ...
                <div className="experience-container">
                  ...
                  <div className="experience-item">
                    <strong>{activeEmployee.education?.title ?? 'Education Qualification'}</strong>
                    <ul>
                  ...
                  <div className="experience-item">
                    <strong>{activeEmployee.awards?.title ?? 'Achievements and Awards'}</strong>
                    <ul>
                  ...
                  <div className="experience-item">
                    <strong>{activeEmployee.experience?.title ?? 'Experience'}</strong>
                    <ul>
...
const ApprovalsPage = ({ data, refetch }: { data: HubInchargeData | null; refetch: () => Promise<void> }) => {
          ...
          <div key={approval.id} className="bg-white shadow rounded-2xl p-4">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
              <div>
                ...
                <p className="text-sm text-gray-600">{approval.type} - ₹{(approval.amount ?? 0).toLocaleString()}</p>
      ...
      <div className="space-y-3">
        {data.purchases.map(purchase => (
          <div key={purchase.id} className="bg-white shadow rounded-2xl p-4">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
              <div>
                <p className="font-semibold">{purchase.vendor ?? 'Unknown vendor'}</p>
                <p className="text-sm text-gray-600">₹{(purchase.amount ?? 0).toLocaleString()}</p>
                <p className="text-xs text-gray-500">{purchase.date ?? '-'}</p>
              </div>
              <span className={`px-2 py-1 rounded text-xs ${
                (purchase.status ?? 'pending') === 'approved' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
              }`}>
...
const ExpensesPage = ({ data, refetch }: { data: HubInchargeData | null; refetch: () => Promise<void> }) => {
  const [newExpense, setNewExpense] = useState<{ amount: string; category: string; remarks: string }>({ amount: '', category: '', remarks: '' });
...
const PerformancePage = ({ data }: { data: HubInchargeData | null }) => {
  if (!data?.performance) return <div className="p-6">Loading performance...</div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white shadow rounded-2xl p-4 h-40 flex flex-col justify-center">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between">
              <span>On-time %:</span>
              <span className="text-green-600">{perf.sla.onTimePercentage}%</span>
            </div>
          </div>
        </div>
      </div>
...
const MessagesPage = ({ data, refetch }: { data: HubInchargeData | null; refetch: () => Promise<void> }) => {
  const [processing, setProcessing] = useState<string | number | null>(null);
...
const CreateTaskPage = ({ refetch }: { refetch: () => Promise<void> }) => {
  const [task, setTask] = useState<{ title: string; details: string; deadline: string; assignedTo: string }>({ title: '', details: '', deadline: '', assignedTo: '' });
  const [submitting, setSubmitting] = useState<boolean>(false);
...
const TasksPage = ({ data, refetch }: { data: HubInchargeData | null; refetch: () => Promise<void> }) => {
  const [processing, setProcessing] = useState<string | number | null>(null);
